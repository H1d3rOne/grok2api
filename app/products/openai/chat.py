...
def _is_tls_error(exc: BaseException) -> bool:
    """Check if an exception is a curl_cffi TLS handshake failure."""
    msg = str(exc).lower()
    return "tls connect error" in msg or "openssl_internal" in msg or "ssl_connect" in msg
...
async def _console_post(
    *,
    ...
    Retries once with a fresh session on TLS handshake failure — a known
    curl_cffi bug where BoringSSL's internal state can become corrupted
    under concurrent use, producing intermittent ``OPENSSL_internal``
    errors that are resolved by creating a new connection.
    """
    proxy = await get_proxy_runtime()
    lease = await proxy.acquire()
    ...
    for attempt in range(2):
        ...
        try:
            ...
            if _is_tls_error(exc) and attempt == 0:
                logger.warning(
                    "console TLS handshake failed (attempt 1), retrying with fresh session: {}",
                    str(exc)[:120],
                )
                continue
    ...
    raise UpstreamError("Console transport failed", status=502)
